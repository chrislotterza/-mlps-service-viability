...
.hero .small {color:#FFFFFF !important; opacity:.90;}
...
def current_inputs(service):
    # Use stored values where present, but fall back to the service/economic
    # defaults for keys that have not yet been created by a conditional widget.
    # This is especially important for alternative-mode inputs (e.g. Air),
    # because Streamlit only instantiates widgets for the currently selected mode.
    values = {
        k: st.session_state.get(k, default)
        for k, default in SERVICES[service].items()
    }
    values.update({
        k: st.session_state.get(k, default)
        for k, default in ECON_DEFAULTS.items()
    })
    return values
...
# conditionally rendered mode inputs start from the intended baselines.
for _key, _default in ECON_DEFAULTS.items():
